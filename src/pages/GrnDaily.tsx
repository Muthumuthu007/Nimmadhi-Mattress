...
const GrnDaily = () => {
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Daily GRN');
      ...
      XLSX.writeFile(wb, `GRN_Daily_${selectedDate}.xlsx`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Daily GRN Report</h1>
              <p className="text-indigo-100 text-sm mt-1">View incoming goods summary for the selected date</p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
        ...
        <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
          <div className="flex-1 min-w-[200px]">
            <input
              type="text"
              className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg focus:border-indigo-500 transition-all"
          ...
          <button
            ...
            disabled={isLoading}
            className={`flex items-center justify-center px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all font-semibold shadow-sm ${isLoading ? 'opacity-75 cursor-not-allowed' : ''}`}
          >
      ...
      {!isLoading && data && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 flex flex-col justify-center items-center">
            <p className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-1">Total count</p>
            <p className="text-4xl font-bold text-gray-900">{data.total_count || records.length}</p>
          </div>
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 flex flex-col justify-center items-center">
            <p className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-1">Total Amount</p>
            <p className="text-4xl font-bold text-indigo-600">₹{(data.total_amount || records.reduce((s, r)=> s + (r.totalAmount || 0), 0)).toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 flex flex-col justify-center items-center">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow border border-gray-200">
          ...
          <p className="mt-1 text-sm text-gray-500">Try a different date or refresh.</p>
        </div>
      ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredRecords.map((record) => (
                  <tr key={record.grnId} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="whitespace-nowrap px-5 py-3 text-sm font-semibold text-indigo-600">₹{(record.totalAmount || 0).toLocaleString()}</td>
