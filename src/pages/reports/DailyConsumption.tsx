...
const DailyConsumption = () => {
  ...
  const fetchConsumption = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        '/api/stock/reports/consumption-summary/',
        {
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `daily-consumption-${selectedDate}.csv`;
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-2xl font-bold dark:text-white">Daily Consumption</h1>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Select a date and click "Generate Report" to view consumption data
          </p>
        </div>
      )}
    </div>
