...
interface OutwardItem {
    ...
    outward_days: { [day: string]: number };
    total_outward: number;
}

interface GridData {
    [category: string]: OutwardItem[];
}
...
const MonthlyOutwardGrid = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            console.log("Making request to: /api/reports/outward/monthly-grid/");
            const response = await axiosInstance.post('/api/reports/outward/monthly-grid/', {
                operation: "GetMonthlyOutwardGrid",
                month: month
            });
    ...
    const handleDownloadExcel = () => {
        ...
        // Title row
        wsData.push([`SOFTFOAM AND LATEX OUT FOR THE MONTH OF ${monthName}`]);
        ...
        Object.entries(data.grid_data).forEach(([category, items]) => {
            ...
            items.forEach((item) => {
                const row = [
                    ...
                    ...daysArray.map(day => item.outward_days[day] || ''),
                    item.total_outward
                ];
                wsData.push(row);
            });
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Monthly Outward');
        XLSX.writeFile(wb, `monthly-outward-grid-${month}.xlsx`);
    };
                    ...
                    <thead>
                        <tr className="bg-yellow-300 dark:bg-yellow-900/50 text-center font-bold border border-gray-400 dark:border-gray-600">
                            <th colSpan={2 + daysInMonth + 1} className="py-2 border border-gray-400 dark:border-gray-600 text-black dark:text-white">
                                SOFTFOAM AND LATEX OUT FOR THE MONTH OF {monthName}
                            </th>
                        </tr>
                            ...
                            <React.Fragment key={category}>
                                ...
                                {items.map((item) => (
                                    <tr key={item.item_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 border border-gray-400 dark:border-gray-600">
                                        ...
                                        {daysArray.map(day => (
                                            <td key={day} className={`border border-gray-400 dark:border-gray-600 px-1 py-1 text-center ${item.outward_days[day] ? 'font-semibold text-gray-900 dark:text-gray-200' : 'text-gray-300 dark:text-gray-600'}`}>
                                                {item.outward_days[day] || ''}
                                            </td>
                                        ))}
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Monthly Outward Grid</h1>
                </div>
            </div>
...
export default MonthlyOutwardGrid;
