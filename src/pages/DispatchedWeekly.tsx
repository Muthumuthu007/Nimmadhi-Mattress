...
const DispatchedWeekly = () => {
  ...
  const handleFetchRecords = () => {
    ...
    if (diffDays > 6) {
      setValidationError('Please select a date range within 7 days.');
      return;
    }
    fetchRecords();
  };
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/reports/production/weekly/', {
        from_date: fromDate,
        to_date: toDate
      });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Weekly Dispatch');
      XLSX.writeFile(wb, `weekly-dispatch-${fromDate}_to_${toDate}.xlsx`);
    } catch (error) {
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Weekly Dispatch Report</h1>
              <p className="text-purple-100 text-sm mt-1">View and manage weekly production dispatches</p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            ...
            <div className="flex items-center gap-2">
              <input
                type="date"
                className="flex-1 px-4 py-2.5 border-2 border-gray-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all"
                value={fromDate}
                onChange={(e) => {
          ...
          <div>
            ...
            <select
              id="status-filter"
              className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all"
              value={statusFilter}
        ...
        <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200 mt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              id="sort-field"
              className="px-4 py-2.5 border-2 border-gray-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all"
              value={sortField}
          ...
          <button
            ...
            disabled={isLoading}
            className={`flex items-center justify-center px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-all shadow-md font-semibold ${isLoading ? 'opacity-75 cursor-not-allowed' : ''
              }`}
          >
            {isLoading ? (
              <Loader2 className="h-5 w-5 mr-2 animate-spin" />
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          >
            <div className="text-left">
              ...
              <p className="text-purple-100 text-sm mt-1">Total dispatched per product · {summary.length} product{summary.length !== 1 ? 's' : ''}</p>
            </div>
            {showSummary
          ...
          {showSummary && (
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {summary.map((s) => (
                  <div key={s.product_id} className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg border-2 border-purple-200 p-4 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <p className="text-xs uppercase tracking-wider text-purple-700 font-semibold mb-1">Product</p>
                        <p className="text-base font-bold text-gray-900">{s.product_name}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs uppercase tracking-wider text-purple-700 font-semibold mb-1">Quantity</p>
                        <p className="text-2xl font-bold text-purple-600">{s.total_quantity}</p>
                      </div>
                    </div>
                  </div>
                ))}
...
export default DispatchedWeekly;