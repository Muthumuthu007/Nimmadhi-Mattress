...
interface WeeklyGroupedDispatchReport {
  from_date: string;
  to_date: string;
...
const DispatchedWeeklyGrouped = () => {
  ...
  const [data, setData] = useState<WeeklyGroupedDispatchReport | null>(null);
  ...
  const handleDownload = () => {
    ...
    try {
      const rows: any[] = [
        [`Weekly Grouped Dispatch Report - ${data.from_date} to ${data.to_date}`],
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-fuchsia-100 text-sm mt-1">Group &rarr; Product breakdown</p>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2">
              <input
                type="date"
                className="flex-1 px-4 py-2.5 border-2 border-gray-300 rounded-lg focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-500/20 transition-all"
                value={fromDate}
          ...
          <div className="flex-1">
            ...
            <input
              type="text"
              className="w-full px-4 py-2.5 border-2 border-gray-300 rounded-lg focus:border-fuchsia-500 transition-all"
      ...
      {!isLoading && data && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 text-center">
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Period</p>
            <p className="text-base font-bold text-gray-900">
              {formatReportDate(data.from_date)} &mdash; {formatReportDate(data.to_date)}
            </p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 text-center">
            <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Groups</p>
            <p className="text-4xl font-bold text-fuchsia-600">{Object.keys(data.grouped_dispatch ?? {}).length}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 text-center">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex items-center gap-5">
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-bold text-fuchsia-700">{total}</p>
                    </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100">
                    ...
                    <div className="px-6 py-3 bg-fuchsia-50 border-t border-fuchsia-100 flex justify-between items-center">
                      <span className="text-sm font-semibold text-fuchsia-700">Group Total</span>
                      <span className="text-lg font-bold text-fuchsia-800">{total}</span>
                    </div>
                        ...
                        {isDetailOpen && (
                          <div className="px-6 py-4 border-t border-gray-100 overflow-x-auto">
                            <table className="min-w-full text-sm">
                              ...
                              <tbody className="divide-y divide-gray-50">
                                {details.map((d, idx) => (
                                  <tr key={d.product_id ? `${d.product_id}-${idx}` : idx} className="hover:bg-fuchsia-50/20 transition-colors">
                                    <td className="py-2.5 text-gray-800">{d.product_name}</td>
                                    <td className="py-2.5 font-bold text-right text-fuchsia-600">{d.quantity_produced}</td>
          ...
          {data && (
            <div className="bg-gradient-to-r from-fuchsia-700 to-pink-700 rounded-xl shadow-md p-5 flex items-center justify-between">
